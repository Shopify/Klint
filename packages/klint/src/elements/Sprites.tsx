...
/**
 * Sprites Element for Klint
 * 
 * Manages spritesheets and sprite rendering
 * 
 * @example
 * ```tsx
 * const preload = async () => {
 *   await K.Sprites.load({
 *     name: 'player',
 ...
 * 
 * const draw = (K) => {
 *   const sheet = K.Sprites.sheet('player');
 *   const frame = Math.floor(K.frame / 10) % sheet.numSprites;
 *   K.Sprites.draw('player', frame, 100, 100);
 * };
 * ```
 */
class Sprites {
  ...
  /**
   * Creates a new Sprites instance
   ...
   */
  async load(...configs: SpriteConfig[]): Promise<void> {
   ...
   */
  private async loadSingle(config: SpriteConfig): Promise<void> {
   ...
   */
  sheet(name: string): Spritesheet | undefined {
...
export class SpriteAnimation {
  ...
  draw(
    x: number,
    y: number,
    options?: Parameters<Sprites['draw']>[4]
  ): void {
    this.sprites.draw(this.sheetName, this.currentFrame, x, y, options);
  }
