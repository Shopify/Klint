...
interface Static {
  curve: Bezier;
  curve2: Bezier;
  lut: ReturnType<Bezier["getLUT"]>;
  bb: ReturnType<Bezier["bbox"]>;
  offPlus: Bezier[];
  offMinus: Bezier[];
  reduced: Bezier[];
  shapes: ReturnType<Bezier["outlineshapes"]>;
  lineHits: string[];
  ccHits: string[];
  length: number;
  pad: number;
...
export default function BezierDemo() {
  ...
  const storage = useStorage<{ s?: Static }>({});
  ...
  const setup = (K: KlintContext) => {
    ...
    storage.set("s", {
      ...
      lut: curve.getLUT(40),
      bb: curve.bbox(),
      offPlus: curve.offset(30) as Bezier[],
      offMinus: curve.offset(-30) as Bezier[],
      reduced: curve.reduce(),
      shapes: curve.outlineshapes(12),
      lineHits: curve.intersects({ p1: { x: pad, y: cy }, p2: { x: w - pad, y: cy } }),
      ccHits: curve.intersects(curve2),
      length: curve.length(),
      pad,
      cy,
      w,
  ...
  const draw = (K: KlintContext) => {
    ...
    if (!s) return;
    const { curve, curve2, lut, bb, offPlus, offMinus, reduced, shapes, lineHits, ccHits, length, pad, cy, w } = s;
    ...
    for (const p of lut) K.circle(p.x, p.y, 2);
    ...
    for (let i = 0; i <= 30; i++) {
      ...
      K.fillColor(c.k > 0 ? "rgba(255,230,109,0.3)" : "rgba(168,230,207,0.3)");
    ...
    for (const h of lineHits) {
      const p = curve.get(parseFloat(h));
      K.circle(p.x, p.y, 5);
    }
  ...
  return (
    <Klint context={context} setup={setup} draw={draw} options={{ fps: 60 }} />
  );
}
