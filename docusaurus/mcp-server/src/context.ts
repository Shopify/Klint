...
export class KlintContext {
  ...
  private parseFunctionInfo(_content: string) {
    // Basic parsing of function exports and types
    const functionMatches = _content.match(/export\s+(?:const|function)\s+(\w+)/g);
    if (functionMatches) {
...
Use the \`explain\` tool to learn more about specific Klint functions!`;
  ...
  private _findRelevantExamples(task: string): string[] {
