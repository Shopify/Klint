...
<KlintPlayground
  code={`function draw(K) {
    ...
    const x = K.width / 2 + Math.sin(K.frame * 0.02) * 100;
    const y = K.height / 2 + Math.cos(K.frame * 0.02) * 100;
...
function draw(K) {
    ...
    for (let i = 0; i < 5; i++) {
        const angle = K.time + (i * Math.PI * 2 / 5);
        const radius = 100;
...
- [Core Concepts](/docs/core-concepts/lifecycle) - Understand lifecycle functions
- [Function Reference](/docs/functions/drawing/circle) - Explore all available functions
