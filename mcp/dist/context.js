...
export class KlintContext {
  ...
  async loadExamples() {
    ...
    try {
      const docComponentsPath = join(
        PROJECT_ROOT,
        "docusaurus/src/components/Experiments"
      );
      const componentFiles = await fs.readdir(docComponentsPath);
      for (const file of componentFiles) {
        if (file.endsWith(".tsx") || file.endsWith(".ts")) {
          const content = await fs.readFile(
            join(docComponentsPath, file),
            "utf-8"
          );
  ...
  async loadFunctionInfo() {
    ...
    try {
      const klintFunctionsPath = join(
        PROJECT_ROOT,
        "lib/src/KlintFunctions.tsx"
      );
      const klintFunctionsContent = await fs.readFile(
        klintFunctionsPath,
        "utf-8"
      );
      ...
      console.warn(
        "Function definitions not available, using documentation-based info"
      );
    }
  }
  ...
  async debug(code, issue) {
    ...
    if (issues.length > 0) {
      ...
      response +=
        "✅ **No obvious issues detected!** Your code looks clean.\n\n";
    }
    ...
    response +=
      "2. **Verify Canvas Rendering** - Ensure the canvas element is visible\n";
    response +=
      "3. **Test Incrementally** - Comment out complex parts and add them back\n";
    response +=
      "4. **Use Console Logging** - Add `console.log()` to track values\n";
    response +=
      "5. **Check Performance** - Use browser dev tools performance tab\n\n";
    // Add common error patterns
    response += "## 🎯 Common Error Patterns\n\n";
    if (code.includes("undefined") || code.includes("null")) {
      response +=
        "❌ **Undefined/Null Values**: Check variable initialization\n";
    }
    ...
    if (!code.includes("ctx.background") && code.includes("draw")) {
      response +=
        "⚠️  **Missing Background**: Consider adding `ctx.background()` to clear the canvas\n";
    }
    if (code.includes("Math.random") && code.includes("draw")) {
      response +=
        "💡 **Random in Draw Loop**: Move random generation to setup or use Time-based patterns\n";
    }
    // Installation check
...
Use the \`explain\` tool to learn more about specific Klint functions!`;
  ...
  findRelevantDocumentation(task, matchedPattern) {
    ...
    const conceptKeywords = lowerTask
      .split(" ")
      .filter((word) => word.length > 3);
    for (const [docName, content] of this.documentation) {
      const lowerContent = content.toLowerCase();
      const isRelevant = conceptKeywords.some(
        (keyword) =>
      ...
      if (content.includes(functionName)) {
        ...
        const relevantLines = lines.filter(
          (line) =>
            ...
            lines.indexOf(line) ===
              lines.findIndex((l) => l.includes(functionName)) - 1 ||
            lines.indexOf(line) ===
              lines.findIndex((l) => l.includes(functionName)) + 1
        );
  ...
  analyzeCode(code) {
    ...
    if (!code.includes("background(")) {
      issues.push({
        ...
        description:
          "No background() call found. This may cause trails or artifacts.",
        suggestion:
          "Add ctx.background(color) at the start of your draw function",
      });
    }
    ...
    if (code.includes("setState") && code.includes("draw")) {
      issues.push({
        type: "React State",
        description:
          "Using setState inside draw() can cause unnecessary re-renders.",
  ...
  checkBestPractices(code) {
    ...
    return (
      practices ||
      "Consider following Klint best practices for better code organization.\n"
    );
  }
