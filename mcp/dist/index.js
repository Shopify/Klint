...
import { z } from "zod";
import { KlintContext } from "./context.js";
...
// Define tool schemas
const HowDoISchema = z.object({
  task: z
    .string()
    .describe("The creative coding task or visual effect you want to achieve"),
  context: z
    .string()
    .optional()
    .describe("Additional context about your project or constraints"),
});
...
const DebugSchema = z.object({
  code: z
    .string()
    .describe("The Klint code that needs debugging or optimization"),
  issue: z
    .string()
    .optional()
    .describe("Description of the problem you're experiencing"),
});
const ShipItSchema = z.object({
  ...
  target: z
    ...
    .describe("Target deployment format"),
});
// Define tools
const tools = [
  {
    name: "how-do-i",
    description:
      "Convert a creative coding task into actionable Klint code patterns and functions",
    inputSchema: {
      type: "object",
      properties: {
        task: {
          type: "string",
          description:
            "The creative coding task or visual effect you want to achieve",
        },
        context: {
          type: "string",
  ...
  },
  {
    name: "explain",
    description:
      "Explain Klint functions with examples and creative possibilities",
    inputSchema: {
      type: "object",
      properties: {
        function: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case "explain": {
        const { function: functionName, includeExamples } =
          ExplainSchema.parse(args);
        const result = await klintContext.explain(
          functionName,
          includeExamples
        );
        return {
